...
// Modified to work in the HAM system

// all votes work on underlying _hamBalances[address], not balanceOf(address)
...
//    Since HAM supply changes, updated quorum & proposal requirements
//    If any uint96, changed to uint256 to match HAM as opposed to comp
...
contract GovernorAlpha {
    ...
    string public constant name = "HAM Governor Alpha";

    /// @notice The number of votes in support of a proposal required in order for a quorum to be reached and for a vote to succeed
    function quorumVotes() public view returns (uint256) { return SafeMath.div(SafeMath.mul(ham.initSupply(), 4), 100); } // 4% of HAM

    /// @notice The number of votes required in order for a voter to become a proposer
    function proposalThreshold() public view returns (uint256) { return SafeMath.div(ham.initSupply(), 100); } // 1% of HAM
        ...
        public
    {
        ...
        Proposal storage proposal = proposals[proposalId];
        require(msg.sender == guardian || ham.getPriorVotes(proposal.proposer, sub256(block.number, 1)) < proposalThreshold(), "GovernorAlpha::cancel: proposer above threshold");
        ...
        internal
    {
        ...
        uint256 votes = ham.getPriorVotes(voter, proposal.startBlock);
