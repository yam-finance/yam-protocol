...
const Voter: React.FC<VoteProps> = () => {
  ...
  const y_vote = useCallback(() => {
    get_y_n_vote3(ethereum, account)
  }, [ethereum, account])

  const fetchVotes = useCallback(async () => {
    getVotes_piece3(ethereum).then(function (data) {
      setTotalVotes(data)
    })
  }, [yam, setTotalVotes])
  ...
  return (
    <Card>
      <CardContent>
        <div style={{ alignItems: 'flex-start', display: 'flex' }}>
          ...
          <StyledCenter>
            ...
            <div style={{
              ...
              <StyledTitle>
                <div>{(Number(totalVotes)* 4).toLocaleString()}</div>
              </StyledTitle>
              <StyledDenominator>
                <div>{`/ 224,746`}</div>
              </StyledDenominator>
        ...
        <StyledMeter>
          <StyledMeterInner width={(Math.max(1000) / 1000 * 100) * Number(totalVotes)* 4 / 200000} />
        </StyledMeter>
        <Spacer />
