...
describe("Distribution", () => {
  ...
  describe("pool failures", () => {
    test("cant join pool 1s early", async () => {
      ...
      expect(yam.toBigN(a["timestamp"]).toNumber()).toBeLessThan(yam.toBigN(starttime).toNumber()); //console.log("starttime", a["timestamp"], starttime);

      await yam.contracts.weth.methods.approve(yam.contracts.eth_pool.options.address, -1).send({
        from: user
      });
      await yam.testing.expectThrow(yam.contracts.eth_pool.methods.stake(yam.toBigN(200).times(yam.toBigN(10 ** 18)).toString()).send({
        from: user,
        gas: 300000
      }), "not start");
      ...
      await yam.contracts.UNIAmpl.methods.approve(yam.contracts.ampl_pool.options.address, -1).send({
        from: user
      });
      await yam.testing.expectThrow(yam.contracts.ampl_pool.methods.stake("5016536322915819").send({
        from: user,
        gas: 300000
      }), "not start");
    });
    ...
    test("cant withdraw more than deposited", async () => {
      ...
      await yam.contracts.eth_pool.methods.stake(yam.toBigN(200).times(yam.toBigN(10 ** 18)).toString()).send({
        from: user,
        gas: 300000
      });
      await yam.contracts.UNIAmpl.methods.approve(yam.contracts.ampl_pool.options.address, -1).send({
      ...
      await yam.testing.expectThrow(yam.contracts.ampl_pool.methods.withdraw("5016536322915820").send({
        from: user,
        gas: 300000
      }), "");
      await yam.testing.expectThrow(yam.contracts.eth_pool.methods.withdraw(yam.toBigN(201).times(yam.toBigN(10 ** 18)).toString()).send({
        from: user,
        gas: 300000
      }), "");
    });
  ...
  describe("incentivizer pool", () => {
    test("joining and exiting", async () => {
      ...
      let rpt = await yam.contracts.eth_pool.methods.rewardPerToken().call(); //console.log(earned, rr, rpt);

      await yam.testing.increaseTime(86400); // await yam.testing.mineBlock();
      ...
      console.log("yam bal", yam_bal); // start rebasing
      ...
      await yam.contracts.uni_router.methods.addLiquidity(yam.contracts.yam.options.address, yam.contracts.ycrv.options.address, yam_bal, yam_bal, yam_bal, yam_bal, user, 1596740361 + 10000000).send({
        from: user,
        gas: 8000000
      });
      ...
      await yam.contracts.ycrv_pool.methods.stake(bal).send({
        from: user,
        gas: 400000
      });
      ...
      rpt = await yam.contracts.ampl_pool.methods.rewardPerToken().call();
      console.log(earned, rr, rpt);
      await yam.contracts.ycrv_pool.methods.exit().send({
        from: user,
        gas: 400000
      });
  ...
  describe("ampl", () => {
    test("rewards from pool 1s ampl", async () => {
      ...
      let rpt = await yam.contracts.ampl_pool.methods.rewardPerToken().call(); //console.log(earned, rr, rpt);

      await yam.testing.increaseTime(625000 + 100); // await yam.testing.mineBlock();

      earned = await yam.contracts.ampl_pool.methods.earned(user).call();
      rpt = await yam.contracts.ampl_pool.methods.rewardPerToken().call();
      let ysf = await yam.contracts.yam.methods.yamsScalingFactor().call(); //console.log(earned, ysf, rpt);
  ...
  describe("eth", () => {
    ...
    test("rewards from pool 1s eth with rebase", async () => {
      ...
      await yam.testing.increaseTime(125000 + 100); // await yam.testing.mineBlock();
      ...
      let pair = await yam.contracts.uni_fact.methods.getPair(yam.contracts.yam.options.address, yam.contracts.ycrv.options.address).call();
      yam.contracts.uni_pair.options.address = pair;
      let bal = await yam.contracts.uni_pair.methods.balanceOf(user).call(); // make a trade to get init values in uniswap
      //console.log("init swap")

      await yam.contracts.uni_router.methods.swapExactTokensForTokens("100000000000000000000000", 100000, [yam.contracts.ycrv.options.address, yam.contracts.yam.options.address], user, 1596740361 + 10000000).send({
        from: user,
        gas: 1000000
      }); // trade back for easier calcs later
      //console.log("swap 0")

      await yam.contracts.uni_router.methods.swapExactTokensForTokens("10000000000000000", 100000, [yam.contracts.ycrv.options.address, yam.contracts.yam.options.address], user, 1596740361 + 10000000).send({
        from: user,
        gas: 1000000
      });
      await yam.testing.increaseTime(43200); //console.log("init twap")
      ...
      await yam.contracts.uni_router.methods.swapExactTokensForTokens("1000000000000000000000", 100000, [yam.contracts.ycrv.options.address, yam.contracts.yam.options.address], user, 1596740361 + 10000000).send({
        from: user,
        gas: 1000000
      }); // init twap

      let init_twap = await yam.contracts.rebaser.methods.timeOfTWAPInit().call(); // wait 12 hours

      await yam.testing.increaseTime(12 * 60 * 60); // perform trade to change price
      //console.log("second swap")

      await yam.contracts.uni_router.methods.swapExactTokensForTokens("10000000000000000000", 100000, [yam.contracts.ycrv.options.address, yam.contracts.yam.options.address], user, 1596740361 + 10000000).send({
        from: user,
        gas: 1000000
      }); // activate rebasing
      ...
      let resycrv = await yam.contracts.ycrv.methods.balanceOf(yam.contracts.reserves.options.address).call(); // new balance > old balance

      expect(yam.toBigN(bal).toNumber()).toBeLessThan(yam.toBigN(bal1).toNumber()); // increases reserves
      ...
      console.log("quote", q); // not below peg
    ...
    test("rewards from pool 1s eth with negative rebase", async () => {
      ...
      console.log("add liq/ create pool");
      await yam.contracts.uni_router.methods.addLiquidity(yam.contracts.yam.options.address, yam.contracts.ycrv.options.address, yam_bal.times(.1).toString(), yam_bal.times(.1).toString(), yam_bal.times(.1).toString(), yam_bal.times(.1).toString(), user, 1596740361 + 10000000).send({
        from: user,
        gas: 8000000
      });
      let pair = await yam.contracts.uni_fact.methods.getPair(yam.contracts.yam.options.address, yam.contracts.ycrv.options.address).call();
      ...
      await yam.contracts.uni_router.methods.swapExactTokensForTokens("1000000000000000000000", 100000, [yam.contracts.yam.options.address, yam.contracts.ycrv.options.address], user, 1596740361 + 10000000).send({
        from: user,
        gas: 1000000
      }); // trade back for easier calcs later
      //console.log("swap 0")

      await yam.contracts.uni_router.methods.swapExactTokensForTokens("100000000000000", 100000, [yam.contracts.yam.options.address, yam.contracts.ycrv.options.address], user, 1596740361 + 10000000).send({
        from: user,
        gas: 1000000
      });
      await yam.testing.increaseTime(43200); //console.log("init twap")
      ...
      await yam.contracts.uni_router.methods.swapExactTokensForTokens("1000000000000000000", 100000, [yam.contracts.yam.options.address, yam.contracts.ycrv.options.address], user, 1596740361 + 10000000).send({
        from: user,
        gas: 1000000
      }); // activate rebasing
  ...
  describe("yfi", () => {
    test("rewards from pool 1s yfi", async () => {
      ...
      await yam.contracts.yfi.methods.approve(yam.contracts.yfi_pool.options.address, -1).send({
        from: user
      });
      ...
      let rpt = await yam.contracts.yfi_pool.methods.rewardPerToken().call(); //console.log(earned, rr, rpt);
  ...
  describe("comp", () => {
    test("rewards from pool 1s comp", async () => {
      ...
      await yam.contracts.comp.methods.approve(yam.contracts.comp_pool.options.address, -1).send({
        from: user
      });
      ...
      let rpt = await yam.contracts.comp_pool.methods.rewardPerToken().call(); //console.log(earned, rr, rpt);
  ...
  describe("lend", () => {
    test("rewards from pool 1s lend", async () => {
      ...
      await yam.contracts.lend.methods.approve(yam.contracts.lend_pool.options.address, -1).send({
        from: user
      });
      ...
      let rpt = await yam.contracts.lend_pool.methods.rewardPerToken().call(); //console.log(earned, rr, rpt);
  ...
  describe("link", () => {
    test("rewards from pool 1s link", async () => {
      ...
      await yam.contracts.link.methods.approve(yam.contracts.link_pool.options.address, -1).send({
        from: user
      });
      ...
      let rpt = await yam.contracts.link_pool.methods.rewardPerToken().call(); //console.log(earned, rr, rpt);
  ...
  describe("mkr", () => {
    test("rewards from pool 1s mkr", async () => {
      ...
      await yam.contracts.mkr.methods.approve(yam.contracts.mkr_pool.options.address, -1).send({
        from: user
      });
      ...
      let rpt = await yam.contracts.mkr_pool.methods.rewardPerToken().call(); //console.log(earned, rr, rpt);
  ...
  describe("snx", () => {
    test("rewards from pool 1s snx", async () => {
      ...
      await yam.contracts.snx.methods.approve(yam.contracts.snx_pool.options.address, -1).send({
        from: user
      });
      ...
      let rpt = await yam.contracts.snx_pool.methods.rewardPerToken().call(); //console.log(earned, rr, rpt);
