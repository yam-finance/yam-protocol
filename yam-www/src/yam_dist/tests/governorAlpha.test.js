...
async function enfranchise(actor, amount, user) {
  await yam.contracts.yam.methods.transfer(actor, amount.toString()).send({
    from: user
  });
  await yam.contracts.yam.methods.delegate(actor).send({
    from: actor
  });
}
...
describe("governorAlpha#castVote/2", () => {
  ...
  let accounts; // we publish the mnemonic. its is a well known test mnemonic so these pvks
  ...
  beforeAll(async () => {
    ...
    a3 = accounts[4];
    a4 = accounts[5];
    let one_hundred = yam.toBigN(100).times(yam.toBigN(10 ** 18)); // await yam.contracts.yam.methods.transfer(guy, one_hundred.toString()).send({from: user});
  ...
  beforeEach(async () => {
    ...
    await yam.contracts.gov.methods.propose(targets, values, signatures, callDatas, "do nothing").send({
      from: a1,
      gas: 400000
    });
  ...
  describe("We must revert if:", () => {
    ...
    test("Such proposal already has an entry in its voters set matching the sender", async () => {
      ...
      await yam.testing.mineBlock();
      await yam.contracts.gov.methods.castVote(proposalId, true).send({
        from: accounts[4]
      });
      await yam.testing.expectThrow(yam.contracts.gov.methods.castVote(proposalId, true).call({
        from: accounts[4]
  ...
  describe("Otherwise", () => {
    ...
    describe("and we take the balance returned by GetPriorVotes for the given sender and the proposal's start block, which may be zero,", () => {
      ...
      test("or AgainstVotes corresponding to the caller's support flag.", async () => {
        ...
        await yam.testing.mineBlock();
        await yam.contracts.gov.methods.castVote(proposalId, false).send({
          from: actor
        });
    ...
    describe('castVoteBySig', () => {
      it('reverts if the signatory is invalid', async () => {
        ...
        await yam.testing.expectThrow(yam.contracts.gov.methods.castVoteBySig(proposalId, false, 0, '0xbad', '0xbad').send({
          from: user
      ...
      it('casts vote on behalf of the signatory', async () => {
        ...
        let beforeFors = (await yam.contracts.gov.methods.proposals(proposalId).call()).forVotes;
        await yam.testing.mineBlock();
        const tx = await yam.contracts.gov.methods.castVoteBySig(proposalId, true, sig.v, sig.r, sig.s).send({
          from: a4,
          gas: 100000
        });
