...
const ethUtil = require('ethereumjs-util'); // import { EIP712Domain } from 'eth-typed-data'
...
describe('YAM governance', () => {
  ...
  describe('delegateBySig', () => {
    ...
    test('reverts if the signatory is invalid', async () => {
      const delegatee = user,
            nonce = 0,
            expiry = 0;
      await yam.testing.expectThrow(yam.contracts.yam.methods.delegateBySig(delegatee, nonce, expiry, 0, '0xbad', '0xbad').send({
        from: a1
    ...
    test('reverts if the nonce is bad ', async () => {
      ...
      const typedData = {
        ...
        domain: {
          name: 'YAM',
          chainId: 1,
          // since we are using --fork, it respects that chainID
      ...
      const sig = ethUtil.ecsign(ethUtil.toBuffer(sigHash, 'hex'), ethUtil.toBuffer(pvk_a1, 'hex')); // const encoded = EIP712.signer.encodeMessageData(delegation.types, delegation.primaryType, delegation.message);

      await yam.testing.expectThrow(yam.contracts.yam.methods.delegateBySig(user, 1, 0, sig.v, sig.r, sig.s).send({
        from: a1
    ...
    test('reverts if the signature has expired', async () => {
      ...
      await yam.testing.expectThrow(yam.contracts.yam.methods.delegateBySig(user, 0, 0, sig.v, sig.r, sig.s).send({
        from: a1
    ...
    test('delegates on behalf of the signatory', async () => {
      ...
      let sigHash = EIP712.encodeTypedData(typedData);
      const sig = ethUtil.ecsign(ethUtil.toBuffer(sigHash, 'hex'), ethUtil.toBuffer(pvk_a1, 'hex'));
      let tx = await yam.contracts.yam.methods.delegateBySig(user, 0, 10e9, sig.v, sig.r, sig.s).send({
        from: a1,
        gas: 500000
      });
    ...
    test('delegate', async () => {
      ...
      let tx = await yam.contracts.yam.methods.delegate(user).send({
        from: a1
      });
      ;
      expect(tx.gasUsed < 80000);
  ...
  describe('numCheckpoints', () => {
    it('returns the number of checkpoints for a delegate', async () => {
      ...
      await yam.contracts.yam.methods.transfer(guy, one_hundred.toString()).send({
        from: user
      });
      let nc = await yam.contracts.yam.methods.numCheckpoints(a1).call();
      expect(nc).toBe('0');
      await yam.contracts.yam.methods.delegate(a1).send({
        from: guy
      });
      nc = await yam.contracts.yam.methods.numCheckpoints(a1).call();
      expect(nc).toBe('1');
      await yam.contracts.yam.methods.transfer(a2, one_hundred.minus(one_hundred.times(.9)).toString()).send({
        from: guy
      });
      nc = await yam.contracts.yam.methods.numCheckpoints(a1).call();
      expect(nc).toBe('2');
      ...
      nc = await yam.contracts.yam.methods.numCheckpoints(a1).call();
      expect(nc).toBe('3');
      await yam.contracts.yam.methods.transfer(guy, one_hundred.minus(one_hundred.times(.8)).toString()).send({
        from: user
      });
      nc = await yam.contracts.yam.methods.numCheckpoints(a1).call();
      expect(nc).toBe('4');
    ...
    it('does not add more than one checkpoint in a block', async () => {
      ...
      let t1 = yam.contracts.yam.methods.delegate(a1).send({
        from: guy
      });
      let t2 = yam.contracts.yam.methods.transfer(a2, yam.toBigN(100).times(yam.toBigN(10 ** 18)).times(.1).toString()).send({
        from: guy
      });
      let t3 = yam.contracts.yam.methods.transfer(a2, yam.toBigN(100).times(yam.toBigN(10 ** 18)).times(.1).toString()).send({
        from: guy
      });
      ...
      let t4 = await yam.contracts.yam.methods.transfer(guy, yam.toBigN(100).times(yam.toBigN(10 ** 18)).times(.2).toString()).send({
        from: user
      });
      nc = await yam.contracts.yam.methods.numCheckpoints(a1).call();
      expect(nc).toBe('2');
  ...
  describe('getPriorVotes', () => {
    ...
    test('returns the latest block if >= last checkpoint block', async () => {
      let t1 = await yam.contracts.yam.methods.delegate(a1).send({
        from: user
      });
    ...
    it('generally returns the voting balance at the appropriate checkpoint', async () => {
      ...
      await yam.testing.mineBlock();
      await yam.testing.mineBlock();
      let t2 = await yam.contracts.yam.methods.transfer(a2, yam.toBigN(100).times(yam.toBigN(10 ** 18)).times(.1).toString()).send({
        from: guy
      });
      await yam.testing.mineBlock();
      await yam.testing.mineBlock();
      let t3 = await yam.contracts.yam.methods.transfer(a2, yam.toBigN(100).times(yam.toBigN(10 ** 18)).times(.1).toString()).send({
        from: guy
      });
      await yam.testing.mineBlock();
      await yam.testing.mineBlock();
